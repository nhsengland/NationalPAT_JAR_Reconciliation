...
-- For assurance, we add in a case statement on Provider codes to make sure that all mergers are correctly captured
	 ...
	 CASE WHEN apc.Der_Provider_Code = 'RD3' THEN 'R0D'
	 WHEN apc.Der_Provider_Code = 'RDZ' THEN 'R0D'
	 WHEN apc.Der_Provider_Code = 'RBZ' THEN 'RH8'
	 WHEN apc.Der_Provider_Code = 'RA3' THEN 'RA7'
	 WHEN apc.Der_Provider_Code = 'RBA' THEN 'RH5'
	 WHEN apc.Der_Provider_Code = 'R1G' THEN 'RA9'
	 WHEN apc.Der_Provider_Code = 'RVJ13' THEN 'RVJ'
	 WHEN apc.Der_Provider_Code = 'RA4' THEN 'RH5'
	 WHEN apc.Der_Provider_Code = 'B4B4S' THEN 'RD1'
	 ELSE apc.Der_Provider_Code END AS 'Der_Provider_Code'
...
-- Isolate the Treatment Function Code and name the column appropriately
	...
	,sp.[Der_Dischg_Treatment_Function_Code] as 'TFC'

-- Isolate the HRG and name the column appropriately

	,apc.Spell_Core_HRG as 'HRG'
...
-- Brings in months in a YYYYMM format
	...
	,YearMonth = CONCAT(YEAR(apc.Discharge_Date),FORMAT(MONTH(apc.Discharge_Date),'00'))
...
-- Case statement to group LOS	
	...
	,case when apc.Der_Episode_LoS = 0 then '0 Day LOS' ELSE '1+Day LOS'END AS 'LOS'

-- Sum all unadjusted activity - As submitted		

	,count(cast(APCE_Ident as varchar)) as 'Total_Activity(Unadj)'

FROM [Reporting_MESH_APC].[APCE_Core_Monthly_Snapshot] as apc
...
LEFT JOIN [Reporting_MESH_APC].[APCS_2526_Der_Monthly_Snapshot] AS Der2526		
	ON apc.APCE_Ident = Der2526.APCS_Ident

--Join to Der table to bring Discharge TFC

LEFT JOIN [Reporting_MESH_APC].[APCS_Core_Monthly_Snapshot] AS Sp	
	ON apc.APCE_Ident = Sp.APCS_Ident
...
-- add in the below if you are looking at a range (for e.g between financial years)
	...
	AND sp.[Der_Dischg_Treatment_Function_Code] NOT IN ('199', '223', '290', '291', '331', '344', '345', '346', '360', '424', '499', '501', '504', '560', '650', '651', '652', '653', '654', '655', '656', '657', '658', '659', '660', '661', '662', '700', '710', '711', '712', '713', '715', '720', '721', '722', '723', '724', '725', '726', '727', '730', '840', '920', 'NULL')       
  -- National planning guidance is specific acute only - excluding some maternity and LDA MH activity
	and apc.Der_Management_Type in ('EL','DC','EM','NE') -- selecting the key elective PODS
...
GROUP BY	
	 ...
	 WHEN apc.Der_Provider_Code = 'RA4' THEN 'RH5'
	 WHEN apc.Der_Provider_Code = 'B4B4S' THEN 'RD1'
	 ELSE apc.Der_Provider_Code END
...
-- For assurance, we add in a case statement on Provider names to make sure that all mergers and name changes are correctly captured
	...
	,o.STP_Name
	,sp.[Der_Dischg_Treatment_Function_Code] 
	,apc.Spell_Core_HRG
...
,case when apc.Der_Episode_LoS = 0 then '0 Day LOS' ELSE '1+Day LOS' END
