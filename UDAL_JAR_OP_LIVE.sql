...
-- For assurance, we add in a case statement on Provider codes to make sure that all mergers are correctly captured
	 ...
	 WHEN op.Der_Provider_Code = 'B4B4S' THEN 'RD1'
...
FROM [Reporting_MESH_OPA].[OPA_Core_Monthly_Snapshot] as op
...
LEFT JOIN [Reporting_MESH_OPA].[OPA_2526_Der_Monthly_Snapshot] AS Der2526					
ON OP.OPA_Ident = Der2526.OPA_Ident


 
WHERE 			
	op.Appointment_Date >= '2025-04-01' -- current FY, YTD
-- add in the below if you are looking at a range (for e.g between financial years)
and op.[Appointment_Date] < '2026-04-01'
...
Else 'Other NHSE' End
,tfc.Treatment_Function_Desc
,CONCAT(YEAR(Appointment_Date),FORMAT(MONTH(Appointment_Date),'00'))
,CASE WHEN Der_Appointment_Type LIKE 'FUp' THEN 'OPAFU' WHEN Der_Appointment_Type LIKE 'New' THEN 'OPFA' END	
