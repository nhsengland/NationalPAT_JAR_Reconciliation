...
,count(cast(apc.[APCS_Ident] as varchar)) as MetricValue
,o.STP_Name
,o.STP_Code

FROM [MESH_APC].[APCS_Core_Daily] as apc 
...
on Der_Dischg_Treatment_Function_Code  = tfc.[Treatment_Function_Code]

left join [MESH_APC].[APCE_Plus_Daily] as a
on apc.APCS_Ident = a.[APCS_Ident]
...
on apc.APCS_Ident = c.[APCS_Ident]
 ...
 where [Discharge_Date] >= '2024-04-01'
    ...
    and c.Responsible_Purchaser_Assignment_Method NOT IN ('Private Patient')	-- CAM Logic to remove private patients ( more inline with national logic and removes issue with ADMIN completeness)
